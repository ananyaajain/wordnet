...
public class TestMultiWordNonZeroKHyponyms {
    ...
    @Test
    public void testK() {
        List<String> words = List.of("marriage");

        NgordnetQuery nq = new NgordnetQuery(words, 1920, 1980, 9);
        String actual = studentHandler.handle(nq);
        String expected = "[marriage, union, wedding]";
        assertEquals(expected, actual);
    }

    @Test
    public void testKAgain() {
        List<String> words = List.of("intake");

        NgordnetQuery nq = new NgordnetQuery(words, 1470, 2019, 9);
        String actual = studentHandler.handle(nq);
        String expected = "[consumption, draft, drink, drinking, eating, feeding, inspiration, intake, pull]";
        assertEquals(expected, actual);
    }
